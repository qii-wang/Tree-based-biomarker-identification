...
bst_optimal = xgboost(data = xgb_df,
                      ...
                      eta = xgboost_para_tune$eta[which.max(xgboost_para_tune$test_auc)],
                      subsample = xgboost_para_tune$subsample[which.max(xgboost_para_tune$test_auc)],
                      colsample.bytree = xgboost_para_tune$colsample_bytree[which.max(xgboost_para_tune$test_auc)],
...
# trian_size = nrow(x_train)
# boot_acc_train_xgb = NULL
# boot_acc_test_xgb = NULL
# for(i in 1:n_boot){
#   boot_id = sample(1:trian_size, trian_size, replace = TRUE)
#   options(na.action='na.pass')
#   boot_x_train_df = model.matrix(~.-1, data = x_train[boot_id,])
#   boot_xgb_df = xgb.DMatrix(data = x_train_df, label = y_train[boot_id])
#   boot_xgb = xgboost(data = boot_xgb_df,
#                       nrounds = xgboost_para_tune$round[which.max(xgboost_para_tune$test_auc)],
#                       max.depth = xgboost_para_tune$max_depth[which.max(xgboost_para_tune$test_auc)],
#                       eta = xgboost_para_tune$eta[which.max(xgboost_para_tune$test_auc)],
#                       subsample = xgboost_para_tune$subsample[which.max(xgboost_para_tune$test_auc)],
...
#                       objective = "binary:logistic",
#                       verbose = 0)
#   boot_y_train_pred_xgb = predict(boot_xgb, newdata = boot_x_train_df)
#   boot_y_test_pred_xgb = predict(boot_xgb, newdata = model.matrix(~.-1, test_dat[,-ncol(test_dat)]))
#   
#   
#   boot_y_train_pred_label_xgb = as.integer(boot_y_train_pred_xgb > opt_thre_xgb)
#   boot_y_test_pred_label_xgb = as.integer(boot_y_test_pred_xgb > opt_thre_xgb)
#   
#   boot_acc_train_xgb[i] = mean(boot_y_train_pred_label_xgb == y_train[boot_id])
#   boot_acc_test_xgb[i] = mean(boot_y_test_pred_label_xgb == test_dat$icans_grade_3plus)
#   
# }
...
bart_para_tune = expand.grid(k = c(1,2,3,4),
                             num_trees = c(20, 50, 100, 150),
                             prob_rule_class = c(0.2, 0.3, 0.5),
                             test_auc = 0)
for(i in 1:nrow(bart_para_tune)){
  cat(i,"\n")
  num_trees = bart_para_tune$num_trees[i]
  k = bart_para_tune$k[i]
  prob_rule_class = bart_para_tune$prob_rule_class[i]
  cv_auc = NULL
  for(j in 1:num_folds){
    ...
    cv_bart = bartMachine(X = cv_x_train, y = as.factor(cv_y_train),
                          num_trees = num_trees,
                          k = k, prob_rule_class = prob_rule_class,
...
bart_optimal = bartMachine(X = x_train, y = as.factor(y_train),
                          ...
                          k = bart_para_tune$k[which.max(bart_para_tune$test_auc)],
                          prob_rule_class = bart_para_tune$prob_rule_class[which.max(bart_para_tune$test_auc)],
                          use_missing_data = TRUE,
                          num_burn_in = 500,
                          num_iterations_after_burn_in = 2000)
...
rf_para_tune = expand.grid(mtry = c(2,3,4,5),
                           nodesize = c(1,3,5),
                           splitrule = c("gini", "auc", "entropy"),
                           sampprop = c(0.5, 0.632, 0.8),
                           test_auc = 0,
...
for(i in 1:nrow(rf_para_tune)){
  ...
  splitrule = rf_para_tune$splitrule[i]
  sampprop = rf_para_tune$sampprop[i]
  cv_auc = NULL
  for(j in 1:num_folds){
    cv_test_id = which(folds == j)
    cv_x_train = x_train[-cv_test_id, ]
    ...
    cv_rf = rfsrc(icans_grade_3plus ~.,
                  ...
                  splitrule = splitrule, sampsize = round(sampprop * nrow(cv_x_train)),
                  na.action = "na.impute")
...
rf_optimal = rfsrc(icans_grade_3plus ~.,
                   ...
                   splitrule = rf_para_tune$splitrule[best_para_id],
                   sampsize = round(rf_para_tune$sampprop[best_para_id] * nrow(x_train)),
                   na.action = "na.impute")
...
library(ggplot2)
pred_score_trian = data.frame(pred_score = c(y_train_pred_xgb, y_train_pred_bart,
                                             y_train_pred_rf, y_train_pred_crf),
                              case = as.factor(rep(y_train, 4)),
                              model = rep(c("XGBoost", "BART", "RF", "CondRF"), each = nrow(train_dat)))
ggplot(pred_score_trian, aes(x = model, y = pred_score, color = case)) +
  geom_boxplot()

pred_score_trian = data.frame(pred_score = c(y_test_pred_xgb, y_test_pred_bart,
                                             y_test_pred_rf, y_test_pred_crf),
                              case = as.factor(rep(test_dat$icans_grade_3plus, 4)),
                              model = rep(c("XGBoost", "BART", "RF", "CondRF"), each = nrow(test_dat)))
ggplot(pred_score_trian, aes(x = model, y = pred_score, color = case)) +
  geom_boxplot()
```